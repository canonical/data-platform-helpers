...
from .helpers import capture, capture_events
...
class DataProvidesBaseTests(ABC):
    ...
    def test_set_credentials_secrets(self):
        ...
        secret_id = requests[0][f"{SECRET_PREFIX}user"]
    ...
    def test_set_entity_credentials(self):
        ...
        response = ResourceProviderModel(
            ...
            request_id="c759221a6c14c72a",
            resource=DATABASE,
            entity_name="test-name",  # pyright: ignore[reportCallIssue]
            entity_password="test-password",  # pyright: ignore[reportCallIssue]
        )
        ...
        requests = json.loads(relation["requests"])
        secret_id = requests[0][f"{SECRET_PREFIX}entity"]
        secret = self.harness.charm.model.get_secret(id=secret_id)
...
class TestDatabaseRequires(DataRequirerBaseTests, unittest.TestCase):
        ...
        self.harness.update_relation_data(
            ...
            self.provider,
            {
                ...
                "requests": json.dumps(
                    [
                        {
                            f"{SECRET_PREFIX}user": secret.id,
                            "salt": "kkkkkkkk",
                            "request-id": "c759221a6c14c72a",
                        }
                    ]
        ...
        self.harness.update_relation_data(
            ...
            self.provider,
            {
                ...
                "requests": json.dumps(
                    [
                        {
                            f"{SECRET_PREFIX}user": secret2.id,
                            "salt": "kkkkkkkk",
                            "request-id": "c759221a6c14c72a",
                        }
                    ]
    ...
    def test_additional_fields_are_accessible(self, _on_resource_created):
        ...
        assert event.response.tls is True
