...
async def test_peer_relation_secrets(component, ops_test: OpsTest):
    ...
    secret = await get_secret_by_label(ops_test, f"database-peers.database.{component}", owner)
    assert secret.get("secret-field") == "blablabla2"
    assert secret.get("monitor-password") is None
...
async def test_secrets_usage_correct_secrets(ops_test: OpsTest):
    ...
    assert request.get("secret-user")
...
async def test_database_roles_relation_with_charm_libraries_secrets(ops_test: OpsTest):
    ...
    requests = json.loads(
        await get_application_relation_data(
            ops_test, APPLICATION_APP_NAME, ROLES_FIRST_DATABASE_RELATION_NAME, "requests"
        )
        or "[]"
    )
    request = requests[0]
...
async def test_relation_secret_revisions(ops_test: OpsTest):
    ...
    original_secret_revision = await get_secret_revision_by_label(
        ops_test, f"{DATABASE_APP_NAME}.{rel_id}.{request_id}.{group_mapping}.secret", owner
    )
...
async def test_provider_get_set_delete_fields_secrets(
    ...
    request = requests[0]
    assert request.get(relation_field) is None
...
async def test_requires_get_set_delete_fields(ops_test: OpsTest):
    ...
    request = requests[0]
    assert request.get("new_field") is None
...
async def test_provider_set_delete_fields_leader_only(ops_test: OpsTest):
    ...
    request = requests[0]
    assert request.get("new_field2") is None
